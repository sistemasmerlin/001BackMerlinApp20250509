...
//Sincronizar existencias, precios y fletes 
    public function sincronizarConTienda(Request $request)
    {
            ...
            ->select("SELECT rtrim(t120.f120_referencia) sku
                ...
                FROM t400_cm_existencia t400
                    ...
                    INNER JOIN t106_mc_criterios_item_mayores t106
                        ...
                        --AND t106.f106_descripcion in ('RINOVA TIRES','CST TIRES','BATERIAS RINOVA','WORCRAFT - RINOVA ATV','HAKUBA - ARMOR - WDT')
                LEFT JOIN
                (
                SELECT
                    t120.f120_id Items
        ...
        return response()->json([
            'productos' => $result,
            'ciudades_fletes' => $ciudades_fletes
        ]);
    }
    ...
    public function guardarPedidoIntegrador(Request $request)
    {
        ...
        $integrador = Integrador::with('user')
            ->where('nit', $data['nit_integrador'])
            ->where('activo', true)
            ->firstOrFail();

        if (Auth::id() && Auth::id() !== (int) $integrador->user_id) {
            return response()->json([
                'ok' => false,
                'mensaje' => 'El token no corresponde al integrador enviado.',
            ], 403);
        }
        ...
        $flete = (float) $data['totales']['flete'];
        ...
        if ($integrador->calcula_flete) {
            ...
            $flete = (float) ($fleteCalculado['flete'] ?? 0);
        }

        $referenciasValidar = collect($data['productos'])
            ->pluck('referencia')
            ->unique()
            ->values()
            ->toArray();
        ...
        $validacion = DB::connection('sqlsrv')->select("
            SELECT 
        ...
        $prefijo = $integrador->prefijo_pedido;
        $orden_compra = $prefijo . $oc;

        $notasOriginal = 'NO ENVIAR FACTURA - Cliente: ' .
            $data['nombre_cliente'] .
            ' - CC/Nit: ' . $data['documento_cliente'] .
            ' - Telefono: ' . $data['telefono_cliente'] .
            ' - Direccion: ' . $data['direccion_envio'] .
            ' (' . $data['nombre_ciudad'] . ' - ' . $data['nombre_departamento'] . ')' .
            ' - OC: ' . $oc;

        if (!empty($data['notas'])) {
            $notasOriginal .= ' - Notas: ' . $data['notas'];
        }
        ...
        try {
            $pedido = Pedido::create([
                'codigo_asesor' => $integrador->codigo_asesor,
                'nombre_asesor' => $integrador->nombre_asesor,
                'nit' => $integrador->nit,
                'razon_social' => $integrador->nombre_comercial ?: optional($integrador->user)->name,
                'lista_precio' => $integrador->lista_precio,
                'estado_siesa' => 'Comprometido',
                'id_estado_pedido' => 1,
                'prefijo' => $prefijo,
                'nota' => $notasLimpias,
                'orden_compra' => $orden_compra,
                'correo_cliente' => optional($integrador->user)->email,
                'estado' => '1',
                'id_sucursal' => $integrador->id_sucursal,
                'flete' => $flete,
                'observaciones' => $notasLimpias,
                'condicion_pago' => $integrador->condicion_pago,
                'fecha_pedido' => now(),
            ]);
            ...
            DireccionEnvio::create([
                'pedido_id' => $pedido->id,
                'id_punto_envio' => $integrador->punto_envio,
                'direccion' => $data['direccion_envio'],
            ...
            $prefijo_siesa = $validacion_siesa[0]->prefijo;
            $consecutivo_siesa = $validacion_siesa[0]->consecutivo;

            $pedido_siesa = $prefijo_siesa . '-' . $consecutivo_siesa;

            $pedido->nota = $pedido_siesa;
            $pedido->save();

            $encabezados = DB::connection('sqlsrv')->select("
                SELECT 
                    CONCAT(bi_t430.[f_id_tipo_docto],' ',bi_t430.[f_nrodocto]) as documento,
                    bi_t430.[f_fecha] as fecha,
                    bi_t430.[f_estado] as estado,
                    bi_t430.f_subtotal_local as f_subtotal,
                    bi_t430.[f_cliente_desp] as nit_cliente,
                ...
                AND bi_t430.f_parametro_biable = '3'
                AND bi_t430.[f_co] = '003'
                AND bi_t430.[f_id_tipo_docto] = ?
                AND bi_t430.[f_nrodocto] = ?
                AND t012.[f012_id_pais] = 169
                AND t013.[f013_id_pais] = 169
                AND t200.f200_ind_estado = 1
                ORDER BY bi_t430.f_nrodocto desc
            ...
            $detalles = DB::connection('sqlsrv')->select("
                ...
                FROM BI_T431 t431
                JOIN t120_mc_items t120 
                    ON t120.f120_rowid = t431.f_rowid_item 
                ...
                JOIN t106_mc_criterios_item_mayores t106 
                    ...
                    AND t106.f106_id_plan = '003' 
                    AND t106.f106_id_cia = '3'
                WHERE t431.f_id_cia = '3'
                AND t431.f_co = '003'
                AND t431.f_parametro_biable = '3'
                AND t431.f_id_tipo_docto = ?
                AND t431.f_nrodocto = ?
                ORDER BY t120.f120_referencia
            ", [
                $prefijo_siesa,
                $consecutivo_siesa,
            ]);
            ...
            try {
                $correosBase = [
                    'ecommerce2@merlinrod.com',
                    'ecommerce@merlinrod.com',
    ...
    private function validarPedidoIntegrador(Request $request): array
    {
        return $request->validate([
            'nit_integrador' => 'required|string|exists:integradores,nit',
