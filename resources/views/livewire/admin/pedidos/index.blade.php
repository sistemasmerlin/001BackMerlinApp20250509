<div class="space-y-6">
    ...
    <div class="w-full mx-auto rounded-xl shadow-lg border border-gray-200 dark:border-zinc-700 p-6">
        ...
        <div class="flex gap-2">
            <form method="GET" class="space-y-4">
                ...
                <div class="flex gap-2">
                    ...
                    <input type="date" id="fecha_inicio" name="fecha_inicio" class="border rounded px-1 py-1"
                        value="{{ request('fecha_inicio') }}">
                    <label for="fecha_final" class="text-sm font-medium">Hasta:</label>
                    <input type="date" id="fecha_final" name="fecha_final" class="border rounded px-1 py-1"
                        value="{{ request('fecha_final') }}">
                    <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1 rounded">Buscar</button>
        ...
        <div wire:ignore>
            <table id="pedidos" class="w-3/4 table-auto text-sm text-left text-gray-700 dark:text-zinc-300"
                ...
                <tbody>
                    @foreach ($pedidos as $pedido)
                        <tr
                            class="bg-white dark:bg-zinc-800 border-b dark:border-zinc-700 hover:bg-gray-50 dark:hover:bg-zinc-700">
                            <td class="px-4 py-2 border-t-2">{{ $pedido->id }}</td>
                            @if ($pedido->prefijo == 'PES')
                                <td class="px-4 py-2 bg-lime-300 border-t-2"><strong>{{ $pedido->prefijo }}</strong>
                                </td>
                            @else
                            ...
                            @if ($pedido->prefijo == 'PES')
                                <td class="px-4 py-2 border-t-2">
                                    ...
                                    <button wire:click="generarNuevaOc({{ $pedido->id }})"
                            ...
                            <td class="px-6 py-4  border-t-2"><strong>{{ $pedido->nit }}</strong> -
                                {{ $pedido->razon_social }}</td>
                            <td class="px-6 py-4 border-t-2">
                                @can('Enviar Pedido Especial')
                                    <div class="flex items-center gap-2">
                                        @if ($pedido->nota === 'Negociación especial')
                                            <flux:modal.trigger name="cambiar-nit"
                                                wire:click="abrirModalNit({{ $pedido->id }})">
                                                <button
                                                    class="px-3 py-1 bg-blue-600 hover:bg-blue-800 text-white font-semibold rounded-lg mt-2">
                            ...
                            <td class="px-6 py-4 border-t-2">
                                @can('Confirmar cliente creado')
                                    <div class="flex items-center gap-2">
                                        @if ($pedido->nota === 'Negociación especial' && $pedido->nit < 500)
                                            <flux:modal.trigger name="confirmar-cliente-creado"
                                                wire:click="abrirModalClienteCreado({{ $pedido->id }})">
                                                <button
                                                    class="px-3 py-1 bg-gray-500 hover:bg-gray-700 text-white font-semibold rounded-lg mt-2">
                                                    <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                        viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                                        class="size-5">
                                                        <path stroke-linecap="round" stroke-linejoin="round"
                                                            d="M21.75 7.5v9a2.25 2.25 0 0 1-2.25 2.25h-15A2.25 2.25 0 0 1 2.25 16.5v-9A2.25 2.25 0 0 1 4.5 5.25h15a2.25 2.25 0 0 1 2.25 2.25zm0 0-9.75 6.75L2.25 7.5" />
                                                    </svg>
                                                </button>
                                            </flux:modal.trigger>
                                        @else
                            ...
                            <td class="px-4 py-2 border-t-2">
                                @can('Enviar Pedido Especial')
                                    <a href="{{ route('admin.pedidos.detalle', $pedido->id) }}"
                                        class="inline-flex px-3 py-1 bg-green-500 hover:bg-green-800 text-white font-semibold rounded-lg">
                                        <svg class="size-5" fill="currentColor" xmlns="http://www.w3.org/2000/svg"
                                            viewBox="0 0 576 512">
                                            <path
                                                d="M288 144a110.9 110.9 0 0 0 -31.2 5 a55.4 55.4 0 0 1 7.2 27 a56 56 0 0 1 -56 56 a55.4 55.4 0 0 1 -27-7.2A111.7 111.7 0 1 0 288 144zm284.5 97.4C518.3 135.6 410.9 64 288 64S57.7 135.6 3.5 241.4a32.4 32.4 0 0 0 0 29.2C57.7 376.4 165.1 448 288 448s230.3-71.6 284.5-177.4a32.4 32.4 0 0 0 0-29.2zM288 400c-98.7 0-189.1-55-237.9-144C98.9 167 189.3 112 288 112s189.1 55 237.9 144C477.1 345 386.7 400 288 400z" />
                                        </svg>
                                    </a>
                                @endcan
                            </td>
                            ...
                            <td class="px-4 py-2 border-t-2">
                                {{ $pedido->direccionEnvio->id_punto_envio ?? 'Sin punto de envío' }}</td>
                            ...
                            @if ($pedido->nota === 'Negociación especial')
                                <td class="px-4 py-2  border-t-2">{{ $pedido->observaciones }}
                                    @can('Enviar Pedido Especial')
                                        <br>
                                        <flux:modal.trigger name="edit-nota"
                                            wire:click="editarNota({{ $pedido->id }})">
                                            <button
                                                class="px-3 py-1 bg-blue-600 hover:bg-blue-800 text-white font-semibold rounded-lg mt-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                            ...
                            <td
                                class="px-4 py-2 {{ $pedido->nota === 'Negociación especial' ? 'bg-lime-300' : 'bg-gray-200' }} border-t-2">
                                <strong>{{ $pedido->nota }}</strong>

                                <br>

                                @can('Editar Estado Pedido')
                                    @if ($pedido->nota !== 'Negociación especial')
                                        <flux:modal.trigger name="edit-estado"
                                            wire:click="editarEstado({{ $pedido->id }})">
                                            <button
                                                class="px-3 py-1 bg-blue-600 hover:bg-blue-800 text-white font-semibold rounded-lg mt-2">
                                                <svg xmlns="http://www.w3.org/2000/svg" fill="none"
                                                    viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"
                                    ...
                                    @if (!str_starts_with($pedido->nota ?? '', 'PAM-'))
                                        <flux:modal.trigger name="edit-pedido-pam" wire:click="editarPedidoPam({{ $pedido->id }})">
                                            <button class="px-3 py-1 bg-amber-600 hover:bg-amber-800 text-white font-semibold rounded-lg mt-2">
                                                Agregar pedido
                                            </button>
                                        </flux:modal.trigger>
                                    @else
                            ...
                            @if ($pedido->nota === 'Negociación especial')
                                <td class="px-4 py-2 border-t-2">
                                    @can('Enviar Pedido Especial')
                                        <button wire:click="eliminarCotizacion({{ $pedido->id }})"
            ...
            <form wire:submit.prevent="guardarNota">
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input id="notaId" wire:model.defer="notaId"
                            class="w-full border bg-gray-300 border-gray-300 rounded p-1" rows="4" readonly>
                    </div>
                    <div>
                        <label for="observacion" class="block text-sm text-gray-900 font-bold">NOTA:</label>
                        <input id="observacion" wire:model.defer="observacion"
                            class="w-full border border-gray-300 rounded p-2" rows="4">
                    </div>
            ...
            <form wire:submit.prevent="guardarEstado">
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <select id="nota" wire:model.defer="nota"
        ...
        <flux:modal name="cambiar-nit" class="md:w-96" x-data x-ref="modalNit"
            ...
            <form wire:submit.prevent="guardarNuevoNit">
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <input class="w-full border bg-gray-200 border-gray-300 rounded p-2"
                            value="{{ $pedidoIdParaNit }}" readonly>
                    </div>

                    <div>
                        <label for="nuevoNit" class="block text-sm text-gray-900 font-bold">NIT:</label>
                        <input id="nuevoNit" wire:model.defer="nuevoNit" type="text" inputmode="numeric"
                            class="w-full border border-gray-300 rounded p-2" placeholder="Ej: 900123456" required />

                        <!-- ========================== -->
                        <!-- Select de Sucursal -->
                        <label for="nuevaSucursal"
                            class="block text-sm text-gray-900 font-bold mt-3">Sucursal:</label>
                        <select id="nuevaSucursal" wire:model.defer="nuevaSucursal"
                        ...
                        <label for="nuevaListaPrecios" class="block text-sm text-gray-900 font-bold mt-3">Lista de
                            precios:</label>
                        <select id="nuevaListaPrecios" wire:model.defer="nuevaListaPrecios"
                            class="w-full border border-gray-300 rounded p-2 bg-white" required>
                        ...
                        <label for="nuevoPuntoEnvio" class="block text-sm text-gray-900 font-bold mt-3">Punto de
                            envío:</label>
                        <select id="nuevoPuntoEnvio" wire:model.defer="nuevoPuntoEnvio"
                            class="w-full border border-gray-300 rounded p-2 bg-white" required>
        ...
        <flux:modal name="confirmar-cliente-creado" class="md:w-96" x-data x-ref="clienteCreado"
            x-on:cerrarModal.window="$refs.confirmarClienteCreado.close()">
            <form wire:submit.prevent="confirmarClienteCreado">
                <div class="space-y-6">
                    <div class="bg-amber-500 p-3 rounded">
                        <h2 class="text-center text-accent font-bold">CONFIRMAR INFORMACIÓN CLIENTE CREADO</h2>
                    </div>
        ...
        <flux:modal name="edit-pedido-pam" class="md:w-96">
            <form wire:submit.prevent="guardarPedidoPam">
                <div class="space-y-6">
                    <div class="bg-amber-500 p-3 rounded">
                    ...
                    <div>
                        <label class="block text-sm text-gray-900 font-bold">ID PEDIDO:</label>
                        <input wire:model.defer="pedidoPamId"
                            class="w-full border bg-gray-300 border-gray-300 rounded p-2" readonly>
                    </div>

                    <div>
                        <label class="block text-sm text-gray-900 font-bold">NÚMERO DE PEDIDO:</label>

                        <div class="flex">
                            <span
                                class="inline-flex items-center px-3 rounded-l border border-r-0 border-gray-300 bg-gray-100 font-bold">
                                PAM
                            </span>

                            <input wire:model.defer="numeroPedidoPam" type="number" min="1" step="1"
                                class="w-full border border-gray-300 rounded-r p-2" placeholder="Ej: 12345" required>
                        </div>

                        @error('numeroPedidoPam')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
        ...
        <script>
            function iniciarDataTable() {
                ...
                $('#pedidos').DataTable({
                    ...
                    "order": [
                        [0, "desc"]
                    ], //Orden de los datos por el ID en la tabla
